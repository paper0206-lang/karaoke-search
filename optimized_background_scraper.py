...
class OptimizedBackgroundScraper:
    def __init__(self, resume_from_checkpoint=True):
        # 設置日誌
        self.setup_logging()
        self.logger = logging.getLogger('OptimizedBackgroundScraper')
        ...
        self.checkpoint_file = "optimized_background_checkpoint.json"
        self.max_singers_per_session = 1000  # 增加到1000
        self.batch_size = 10  # 增加到10
        self.delay_range = (2.0, 4.0)  # 減少延遲
        self.parallel_threads = 4  # 並行處理 (增加到4線程)
        
        # Git自動推送配置
        self.auto_git_push = True
        self.git_push_interval = 50  # 每50位歌手推送一次
        ...
        # 創建輸出目錄
        os.makedirs("optimized_scraping_results", exist_ok=True)
        
        self.logger.info(f"🚀 優化版背景爬蟲初始化完成")
        ...
        self.logger.info(f"優化延遲: {self.delay_range}")
        self.logger.info(f"自動Git推送: {'啟用' if self.auto_git_push else '停用'}")
        
    def setup_logging(self):
        """設置日誌系統"""
        log_dir = "optimized_logs"
        os.makedirs(log_dir, exist_ok=True)
        
        log_file = f"{log_dir}/optimized_scraper_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    ...
    def _load_singers_list(self):
        ...
        try:
            ...
            # 🎯 智能排序：按基準分數排序，優先處理高分歌手
            def get_benchmark_score(singer):
                try:
                    singer_info = singers_data.get(singer, {})
            ...
            # 按基準分數降序排序
            singers_list.sort(key=get_benchmark_score, reverse=True)
            
            self.logger.info(f"載入歌手清單: {len(singers_list)} 位 (按基準分數排序)")
            return singers_list
    ...
    def search_singer_parallel(self, singer_name):
        """並行搜索單個歌手的資料"""
    ...
    def auto_git_push_update(self, singer_name, force=False):
        ...
        try:
            ...
            commit_message = f"🚀 優化版爬蟲更新: 處理{self.stats['processed_singers']}位歌手，新增{self.stats['new_songs_added']}首歌"
    ...
    def process_singer(self, singer_name):
        ...
        try:
            ...
            # 🚀 使用並行搜索
            new_songs, companies_found, raw_data = self.search_singer_parallel(singer_name)
    ...
    def run_optimized_scraping(self):
        """執行優化版爬取"""
        self.logger.info("=" * 60)
        self.logger.info("🚀 優化版背景大規模爬蟲開始執行")
        ...
        for batch_start in range(0, len(self.singers_to_process), self.batch_size):
            ...
            self.logger.info(f"\n--- 批次 {batch_num + 1}/{total_batches} (並行處理) ---")
            ...
            with ThreadPoolExecutor(max_workers=self.parallel_threads) as executor:
                ...
                for i, future in enumerate(as_completed(future_to_singer), 1):
                    if self.shutdown_requested:
                        break
                    ...
                    self.logger.info(f"[{i:2d}/{len(batch_singers)}] 完成: {singer}")
...
def main():
    ...
    scraper = OptimizedBackgroundScraper(resume_from_checkpoint=resume)
    
    try:
        results = scraper.run_optimized_scraping()
        ...
        if results:
            ...
            print(f"📊 詳細日誌: optimized_logs/ 目錄")
        else:
            print(f"\n✅ 所有歌手都已達到95%基準標準！")
