...
def get_latest_checkpoint():
    """獲取最新檢查點資訊"""
    checkpoint_file = "fixed_background_checkpoint.json"
...
def get_latest_logs():
    """獲取最新日誌信息"""
    log_files = glob.glob("fixed_logs/*.log")
...
def monitor_progress():
    ...
    # 檢查批次結果
    batch_files = glob.glob("fixed_scraping_results/batch_*_progress.json")
    if batch_files:
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--continuous':
        ...
        monitor_progress()
        print(f"\n💡 提示:")
        print(f"   連續監控: python3 monitor_fixed.py --continuous")
        print(f"   查看日誌: tail -f fixed_logs/*.log")
        print(f"   停止爬蟲: ./stop_fixed_scraper.sh")
